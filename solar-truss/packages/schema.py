...
class ModelInput:
    def __init__(
        ...
        self._top_p = top_p
        self._top_k = top_k
        self._random_seed = random.randint(0, 1000000000) if random_seed is None else random_seed
        # Max new tokens is passed by the OpenAI proxy but underlying TensorRT-LLM
        # engine does not support this param, onyl the max_tokens param.
    ...
    def to_tensors(self):
        ...
        inputs = [
            ...
            self._prepare_grpc_tensor("random_seed", random_seed_data),
        ]
