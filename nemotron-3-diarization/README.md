...
serving engine built around NVIDIA's NeMo streaming loop, so throughput and concurrency exceed
...
Deploy any preset in one click from the Baseten Model Library; each deploy link creates the model in
your workspace on an NVIDIA RTX PRO 6000:

- Batch diarization — <https://app.baseten.co/deploy/baseten/nemotron-3-diarization-batch>
- Streaming diarization — <https://app.baseten.co/deploy/baseten/nemotron-3-diarization-streaming>
- Real-time diarized transcription — <https://app.baseten.co/deploy/baseten/nemotron-3-diarized-transcription-streaming>

The checkpoint (`nvidia/Nemotron-3-Diarization`, OpenMDW 1.1) is fetched at deploy time.
