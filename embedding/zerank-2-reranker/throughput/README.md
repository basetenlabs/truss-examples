# BEI (Baseten-Embeddings-Inference) with zeroentropy/zerank-2-reranker

This is a Deployment for BEI (Baseten-Embeddings-Inference) with zeroentropy/zerank-2-reranker. BEI is Baseten's solution for production-grade deployments via TensorRT-LLM for (text) embeddings, reranking models and prediction models.
...
# Examples:
This deployment is specifically designed for the Hugging Face model [baseten-admin/zerank-2-reranker-seq](https://huggingface.co/baseten-admin/zerank-2-reranker-seq).
...
baseten-admin/zerank-2-reranker-seq is a sequence-classification reranker derived from [zeroentropy/zerank-2-reranker](https://huggingface.co/zeroentropy/zerank-2-reranker) by converting its single-token relevance head into a `score` head, so it can be served as a standard cross-encoder reranker. It returns a single relevance logit per (query, document) pair.
...
cd 11-embeddings-reranker-classification-tensorrt/BEI-zerank-2-reranker-seq-fp8
```

With `11-embeddings-reranker-classification-tensorrt/BEI-zerank-2-reranker-seq-fp8` as your working directory, you can deploy the model with the following command. Paste your Baseten API key if prompted.

```sh
truss push --publish
# prints:
# ✨ Model BEI-zerank-2-reranker-seq-fp8-truss-example was successfully pushed ✨
...
model_metadata:
  example_model_input:
    ...
    truncate: true
    truncation_direction: Right
model_name: BEI-zerank-2-reranker-seq-fp8-truss-example
python_version: py39
resources:
