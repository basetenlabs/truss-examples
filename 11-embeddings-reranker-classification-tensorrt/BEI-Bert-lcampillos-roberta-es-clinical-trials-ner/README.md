# BEI-Bert (Baseten-Embeddings-Inference-BERT) with lcampillos/roberta-es-clinical-trials-ner

This is a Deployment for BEI-Bert (Baseten-Embeddings-Inference-BERT) with lcampillos/roberta-es-clinical-trials-ner. BEI is Baseten's solution for production-grade deployments via TensorRT-LLM for (text) embeddings, reranking models and prediction models.
...
# Examples:
This deployment is specifically designed for the Hugging Face model [lcampillos/roberta-es-clinical-trials-ner](https://huggingface.co/lcampillos/roberta-es-clinical-trials-ner).
...
cd 11-embeddings-reranker-classification-tensorrt/BEI-Bert-lcampillos-roberta-es-clinical-trials-ner
```

With `11-embeddings-reranker-classification-tensorrt/BEI-Bert-lcampillos-roberta-es-clinical-trials-ner` as your working directory, you can deploy the model with the following command. Paste your Baseten API key if prompted.

```sh
truss push --publish
# prints:
# ✨ Model BEI-Bert-lcampillos-roberta-es-clinical-trials-ner-truss-example was successfully pushed ✨
...
model_metadata:
  example_model_input:
    ...
    raw_scores: false
    truncate: true
model_name: BEI-Bert-lcampillos-roberta-es-clinical-trials-ner-truss-example
python_version: py39
resources:
  accelerator: L4
  cpu: '1'
...
trt_llm:
  build:
    ...
    checkpoint_repository:
      repo: lcampillos/roberta-es-clinical-trials-ner
      revision: main
      source: HF
