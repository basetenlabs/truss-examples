# BEI (Baseten-Embeddings-Inference) with nomic-ai/nomic-embed-code

This is a Deployment for BEI (Baseten-Embeddings-Inference) with nomic-ai/nomic-embed-code. BEI is Baseten's solution for production-grade deployments via TensorRT-LLM for (text) embeddings, reranking models and prediction models.
...
# Examples:
This deployment is specifically designed for the Hugging Face model [nomic-ai/nomic-embed-code](https://huggingface.co/nomic-ai/nomic-embed-code).
...
nomic-ai/nomic-embed-code  is a text-embeddings model, producing a 1D embeddings vector, given an input.
...
cd 11-embeddings-reranker-classification-tensorrt/BEI-nomic-ai-nomic-embed-code-fp8
```

With `11-embeddings-reranker-classification-tensorrt/BEI-nomic-ai-nomic-embed-code-fp8` as your working directory, you can deploy the model with the following command. Paste your Baseten API key if prompted.

```sh
truss push --publish
# prints:
# ✨ Model BEI-nomic-ai-nomic-embed-code-fp8-truss-example was successfully pushed ✨
...
model_metadata:
  example_model_input:
    ...
    input: text string
    model: model
model_name: BEI-nomic-ai-nomic-embed-code-fp8-truss-example
python_version: py39
resources:
