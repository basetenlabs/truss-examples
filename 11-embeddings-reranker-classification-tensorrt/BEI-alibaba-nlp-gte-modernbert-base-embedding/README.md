# BEI (Baseten-Embeddings-Inference) with Alibaba-NLP/gte-modernbert-base-embedding

This is a Deployment for BEI (Baseten-Embeddings-Inference) with Alibaba-NLP/gte-modernbert-base-embedding. BEI is Baseten's solution for production-grade deployments via TensorRT-LLM for (text) embeddings, reranking models and prediction models.
...
cd 11-embeddings-reranker-classification-tensorrt/BEI-alibaba-nlp-gte-modernbert-base-embedding
```

With `11-embeddings-reranker-classification-tensorrt/BEI-alibaba-nlp-gte-modernbert-base-embedding` as your working directory, you can deploy the model with the following command. Paste your Baseten API key if prompted.

```sh
truss push --publish
# prints:
# ✨ Model BEI-alibaba-nlp-gte-modernbert-base-embedding-truss-example was successfully pushed ✨
...
model_metadata:
  example_model_input:
    ...
    input: text string
    model: model
model_name: BEI-alibaba-nlp-gte-modernbert-base-embedding-truss-example
python_version: py39
resources:
  accelerator: L4
  cpu: '1'
