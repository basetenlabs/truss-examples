# BEI (Baseten-Embeddings-Inference) with nomic-ai/nomic-embed-text-v2-moe

This is a Deployment for BEI (Baseten-Embeddings-Inference) with nomic-ai/nomic-embed-text-v2-moe. BEI is Baseten's solution for production-grade deployments via TensorRT-LLM for (text) embeddings, reranking models and prediction models.
...
cd 11-embeddings-reranker-classification-tensorrt/BEI-nomic-ai-nomic-embed-text-v2-moe
```

With `11-embeddings-reranker-classification-tensorrt/BEI-nomic-ai-nomic-embed-text-v2-moe` as your working directory, you can deploy the model with the following command. Paste your Baseten API key if prompted.

```sh
truss push --publish
# prints:
# ✨ Model BEI-nomic-ai-nomic-embed-text-v2-moe-truss-example was successfully pushed ✨
...
model_metadata:
  example_model_input:
    ...
    input: text string
    model: model
model_name: BEI-nomic-ai-nomic-embed-text-v2-moe-truss-example
python_version: py39
resources:
  accelerator: L4
  cpu: '1'
