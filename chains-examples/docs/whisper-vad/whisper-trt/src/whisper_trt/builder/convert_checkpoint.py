# Taken from the TRT-LLM repo at git sha `5ddb6bf`
# TensorRT-LLM/examples/whisper/convert_checkpoint.py
...
def get_decoder_config(
    model_metadata: dict, dtype: str, logits_dtype: str, quant_algo: QuantAlgo
) -> dict:
    return {
...
def convert_openai_whisper_decoder(
    model_metadata: dict, model_params: dict, quant_algo: str = None
):
    ...
    def convert_and_save(component: str = "encoder"):
        ...
        if component == "encoder":
            ...
            config = get_decoder_config(
                model_metadata, args.dtype, args.logits_dtype, quant_algo
            )
