...
def send_one_batch(base_url, num_prompts, batch_size, cli_args):
    padded_prompts = (prompts * ((num_prompts + len(prompts) - 1) // len(prompts)))[
        :num_prompts
    ]
    # check if cli_args input and output length exist
    if cli_args.input_len:
        input_len = cli_args.input_len
    else:
        input_len = 0
    if cli_args.output_len:
        output_len = cli_args.output_len
    else:
        output_len = 512
    # format: (prompt, input_len, output len). We set input_len as a dummy value 0.
    input_requests = [(p, input_len, output_len) for p in padded_prompts]
...
def main(args, server_args):
    ...
    for i in range(args.start, args.end or len(configs)):
        ...
        try:
            # Warmup
            send_one_batch(base_url, batch_size, batch_size, args)

            # Benchmark
            acc_length, step_time, speed, completion_tokens = send_one_batch(
                base_url, max(args.num_prompts, batch_size), batch_size, args
            )
        finally:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-len", type=int, default=0, help="Input tokens")
    parser.add_argument("--output-len", type=int, default=512, help="Output tokens")
