...
class Model:
    ...
    def predict(self, model_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"[D] image_base64: {type(image_base64)} = {str(image_base64)[:50] if image_base64 else None}...")
            print(f"[D] image_urls: {type(image_urls)} = {image_urls}")
            print(f"[D] image_base64_list: {type(image_base64_list)} = {len(image_base64_list) if image_base64_list else 0} items")
            print(f"[D] task: {task}")
            ...
            if isinstance(image_url, list) and len(image_url) > 0:
                print(f"[I] Detected list in image_url, treating as image_urls")
            ...
            if image_urls:
                ...
                if not isinstance(image_urls, list):
                    raise ValueError(f"image_urls must be a list, got {type(image_urls)}: {image_urls}")
                ...
                if not isinstance(image_base64_list, list):
                    raise ValueError(f"image_base64_list must be a list, got {type(image_base64_list)}")
                ...
                print(f"[I] Processing single base64 image")
                images.append(self._load_base64_image(image_base64))
            else:
                raise ValueError("No valid image input provided. Use image_url, image_base64, image_urls, or image_base64_list")
            
            # Validate batch size
            if len(images) > batch_size:
                print(f"[W] Warning: {len(images)} images provided but batch_size is {batch_size}. Processing first {batch_size} images.")
                images = images[:batch_size]
            elif len(images) < batch_size:
                print(f"[W] Warning: {len(images)} images provided but batch_size is {batch_size}. This may not be optimal for GPU utilization.")
            ...
            with torch.no_grad():
                ...
                if task == "classification":
                    embeddings = self._extract_classification_embedding(last_hidden_state)
                elif task == "segmentation":
                    embeddings = self._extract_segmentation_embedding(last_hidden_state)
                else:
                    raise ValueError(f"Unsupported task: {task}. Use 'classification' or 'segmentation'")
                ...
                return {
                    ...
                    "gpu_memory_gb": torch.cuda.get_device_properties(0).total_memory / (1024**3) if self.device == "cuda" else 0
                }
    ...
    def _load_image(self, image_url: str) -> Image.Image:
        ...
        try:
            ...
            if not isinstance(image_url, str):
                raise ValueError(f"image_url must be a string, got {type(image_url)}: {image_url}")
            ...
            if e.response.status_code == 403:
                raise ValueError(f"Access forbidden (403) for {image_url}. This may be due to missing User-Agent or access restrictions. Consider using image_base64 instead.")
            else:
                raise ValueError(f"HTTP error {e.response.status_code} for {image_url}: {e}")
