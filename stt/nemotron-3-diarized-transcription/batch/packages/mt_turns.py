...
class TurnBuilder:
    ...
    def __init__(self, pause_s=1.2, sentence_break=True):
    ...
    def advance(self, processed_s):
        """Silence after the open tail longer than the pause: close it so it gets promoted."""
        if self.open is not None and processed_s - self.open.end > self.pause_s:
            self._close()
...
class OverlapTurnBuilder(TurnBuilder):
    ...
    def __init__(self, pause_s=1.2, sentence_break=True, handover_s=1.0, handover_words=3):
        super().__init__(pause_s, sentence_break)
    ...
    def advance(self, processed_s):
        for seg in list(self.open.values()):
            if processed_s - seg.end > self.pause_s:
                self._close_seg(seg)
...
class SessionTurns:
    ...
    def __init__(self, inc, flags: TurnFlags, words=None, overlap=None):
        ...
        if flags.overlap if overlap is None else bool(overlap):
            ...
            self.tb = TurnBuilder(flags.pause_s, flags.sentence_break)
