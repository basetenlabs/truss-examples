# (pankaj) Repurposed from https://github.com/NVIDIA/TensorRT-LLM/blob/main/examples/enc_dec/run.py
import json
import time
...
class TRTLLMEncDecModel:
    ...
    def process_input(
        ...
        if remove_input_padding:
            ...
            ils = 1 + (input_ids[:, 1:] != pad_token_id).sum(dim=1).type(
                torch.IntTensor
            ).to(self.device)
            input_lengths = ils.clone().detach().to(self.device).to(torch.int32)
    ...
    def generate(
        ...
        if attention_mask is not None:
            cross_attention_mask = (
                attention_mask.clone().detach().to(self.device).to(torch.int32)
            )
            cross_attention_mask = cross_attention_mask.reshape(
                attention_mask.shape[0], 1, attention_mask.shape[1]
            )
        ...
        sampling_config.update(output_cum_log_probs=False, output_log_probs=False)
