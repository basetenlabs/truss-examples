...
class Model:
    ...
    def load(self):
        ...
        self._tllm_model = TRTLLMEncDecModel.from_engine(self._engine_name, self._engine_dir)
    ...
    def predict(self, model_input):
        try:
            ...
            tokenized_inputs = self._tokenizer(input_text, return_tensors="pt", padding=True)
            input_ids = tokenized_inputs.input_ids.type(torch.IntTensor).to('cuda')
            decoder_input_ids = torch.IntTensor([[self._decoder_start_token_id]]).to('cuda')
