...
def model_wrapper(
    ...
    def get_model_input_time(t_continuous):
        ...
        if noise_schedule.schedule == "discrete":
            return (
                t_continuous - 1.0 / noise_schedule.total_N
            ) * noise_schedule.total_N
        ...
        if guidance_tp == "uncond":
            ...
            if (
                ...
                try:
                    ...
                    noise_uncond, noise = noise_pred_fn(x_in, t_in, cond=c_in)[0].chunk(
                        2
                    )
            ...
            for i in pag_applied_layers:
                if isinstance(model, torch.nn.Module):
                    ...
                    model.__self__.blocks[i].attn.forward = (
                        ...
                        if guidance_scale == 1.0
                        else PAGCFGIdentitySelfAttnProcessorLiteLA(
                            model.__self__.blocks[i].attn
                        )
                    )
            ...
            c_in = torch.cat(
                [condition, condition]
                if guidance_scale == 1.0
                else [unconditional_condition, condition, condition]
            )
            ...
            if guidance_scale == 1.0:
                ...
                noise_pred = (
                    noise_uncond
                    + guidance_scale * (noise - noise_uncond)
                    + pag_scale * (noise - noise_perturb)
                )
            for i in pag_applied_layers:
                if isinstance(model, torch.nn.Module):
                    model.blocks[i].attn.forward = SelfAttnProcessorLiteLA(
                        model.blocks[i].attn
                    )
                else:
                    model.__self__.blocks[i].attn.forward = SelfAttnProcessorLiteLA(
                        model.__self__.blocks[i].attn
                    )
            ...
            if t_continuous[0] < 0.5:
                ...
                if (
                    ...
                    or unconditional_condition is None
                    or not (
                        interval_guidance[0] < t_continuous[0] < interval_guidance[1]
                    )
                ):
                    return noise_pred_fn(x, t_continuous, cond=condition)
                ...
                try:
                    noise_uncond, noise, noise_perturb = noise_pred_fn(
                        x_in, t_in, cond=c_in
                    ).chunk(3)
                except:
                    noise_uncond, noise, noise_perturb = noise_pred_fn(
...
class DPM_Solver:
    ...
    def register_progress_bar(self, progress_fn=None):
        ...
        """
        self.progress_fn = (
            progress_fn if progress_fn is not None else lambda step, total: None
        )
