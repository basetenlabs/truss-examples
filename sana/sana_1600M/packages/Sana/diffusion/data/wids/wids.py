...
class IndexedTarSamples:
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            print(
                f"[wids-debug] curr idx: {idx}, total sample length: {len(self.samples)} {e}"
            )
            raise e
        sample = {}
        key = None
...
class ShardListDataset(Dataset[T]):
        ...
        *,
        ...
        if isinstance(shards, (str, io.IOBase)):
            ...
            raise NotImplementedError(
                "Only support taking path/url to JSON descriptor file."
            )
            self.base = None
            self.spec = options
            self.shards = shards
    ...
    def check_cache_misses(self):
        ...
        if accesses > 100 and misses / accesses > 0.3:
            ...
            print(
                f"Warning: ShardListDataset has a cache miss rate of {misses * 100.0 / accesses:.1%}%"
            )
        ...
        if url.startswith(("https://", "http://", "gs://", "/", "~")):
            ...
            if self.base is None and "base_path" not in self.spec:
                raise FileNotFoundError(
                    "passing a relative path in shardlist but no base found."
                )
            base_path = (
                self.spec["base_path"] if "base_path" in self.spec else self.base
            )
    ...
    def __init__(
        ...
        *,
        ...
        if sort_data_inseq and len(self.spec.get("shardlist", [])) > 0:
            ...
            self.spec["shardlist"] = split_and_recombine(
                self.spec["shardlist"], num_replicas
            )
...
def split_and_recombine(lst, n):
    ...
    unique_parts = list(
        OrderedDict((extract_prefix(item), None) for item in lst).keys()
    )
