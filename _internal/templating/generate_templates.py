...
# Map each solution type to its output subdirectory
SUBFOLDER_MAP = {
    "BEI": Path("embeddings/bei"),
    "BEI-Bert": Path("embeddings/bei"),
    "TEI": Path("embeddings/tei"),
    "Briton": Path("optimized/briton"),
    "BISV2": Path("optimized/bisv2"),
}
...
def generate_deployment(dp: Deployment):
    ...
    subfolder = SUBFOLDER_MAP.get(dp.solution.nickname)
    if subfolder is None:
        raise ValueError(
            f"No SUBFOLDER_MAP entry for solution nickname '{dp.solution.nickname}'"
        )
    # Strip the solution nickname prefix from the folder name since directory
    # names no longer include the BEI-/TEI-/Briton-/BISV2- prefix
...
"""
    # Write generated README to _internal/ (not repo root — that's manually maintained)
    (Path(__file__).parent / "GENERATED_README.md").write_text(readme)
    print(readme)
