...
class TRTClip:
    def __init__(self, orig_clip, engine_path, stream=None):
        # Load trt engine
    ...
    def load(self):
        ...
        self._tensors["text_embeddings"] = torch.empty(
            (1, 77, 768), dtype=torch.float, device="cuda"
        )
    ...
    def __call__(self, *args, **kwargs):
        ...
        hf_model_hidden_states = [
            torch.empty((1, 77, 768), dtype=torch.float, device="cuda")
        ] * 13
        hf_model_hidden_states[-2] = self._tensors["hidden_states"].clone()
        return BaseModelOutputWithPooling(
            last_hidden_state=self._tensors["text_embeddings"].clone(),
