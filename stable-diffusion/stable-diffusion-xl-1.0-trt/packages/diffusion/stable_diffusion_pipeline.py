...
class StableDiffusionPipeline:
    ...
    def __init__(
        ...
        # TODO(pankaj) Support non-classifier-free-guidance. Hardcoded to true right now.
        # Supporting this will involve moving this handling to inference time, which will
        # involve updating the models as well, as this is passed during construction of 
        # those models right now.
    ...
    def infer(
        ...
        Args:
            ...
            denoising_end (float): 
                Determines the fraction (between 0.0 and 1.0) of the total denoising process to be completed 
                before it is intentionally prematurely terminated. As a result, the returned sample will still
                retain a substantial amount of noise (ca. final 20% of timesteps still needed) and should be 
                denoised by a successor pipeline that has denoising_start set to 0.8 so that it only denoises 
                the final 20% of the scheduler. The denoising_end parameter should ideally be utilized when this 
                pipeline forms a part of a “Mixture of Denoisers” multi-pipeline setup.
            refiner_denoising_start (float):
                Only applycable to refiner pipeline type.
                Determines the the fraction (between 0.0 and 1.0) of the total denoising process to be bypassed 
                before it is initiated. Consequently, the initial part of the denoising process is skipped and 
                it is assumed that the passed image is a partly denoised image. Note that when this is specified,
                strength will be ignored.
            guidance_scale (float):
                # TODO(pankaj) Support < 1 case. For now this is required to be > 1.
                Guidance scale is enabled by setting as > 1. 
                Higher guidance scale encourages to generate images that are closely linked to the text prompt, 
                usually at the expense of lower image quality.
        """
        assert(guidance_scale > 1)  # pankaj: For now we only support guidance > 1
        ...
        with torch.inference_mode(), torch.autocast("cuda"), trt.Runtime(TRT_LOGGER):
            ...
            if timesteps is not None:
                ...
                assert denoising_steps == len(self.scheduler.timesteps)
            else:
                self.scheduler.set_timesteps(denoising_steps, device=self.device)
            timesteps = self.scheduler.timesteps.to(self.device)

            denoise_kwargs = {'guidance_scale': guidance_scale}
