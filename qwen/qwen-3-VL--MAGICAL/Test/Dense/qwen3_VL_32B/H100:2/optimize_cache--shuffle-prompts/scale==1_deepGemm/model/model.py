...
_sglang_client_initialized = False  # Track if we've logged initialization


def _init_sglang_client() -> httpx.AsyncClient:
    """
    Initialize the SGLang HTTP client with connection pooling.
    Uses keep-alive connections to reduce connection overhead.
    """
    global _sglang_client, _sglang_client_initialized
    
    if _sglang_client is None:
        logger.info("Initializing SGLang HTTP client with connection pool...")
        _sglang_client = httpx.AsyncClient(
            ...
            limits=httpx.Limits(
                ...
                max_keepalive_connections=50,  # Keep-alive connections for reuse
                keepalive_expiry=30.0          # Keep connections alive for 30s
            ),
            http2=False,  # HTTP/1.1 for simpler connection reuse
        )
        logger.info(f"SGLang client initialized: max_connections=200, keepalive=50")
        _sglang_client_initialized = True
...
async def _request_with_retry(
    ...
    """
    Make HTTP request with retry logic for transient connection failures.
    ...
    for attempt in range(max_retries):
        try:
            if attempt > 0:
                logger.info(f"Retry attempt {attempt + 1}/{max_retries} for {method} {url}")
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning(
                    f"Connection failed (attempt {attempt + 1}/{max_retries}): {type(e).__name__}. "
                    f"Retrying in {wait_time:.2f}s..."
                )
                await asyncio.sleep(wait_time)
            else:
                logger.error(
                    f"All {max_retries} connection attempts failed for {method} {url}: {e}"
                )
        except Exception as e:
            # Don't retry on non-connection errors
            logger.error(f"Non-retryable error during request: {type(e).__name__}: {e}")
            raise
...
class Model:
    ...
    def load(self):
        ...
        # Wait for model_cache download to complete before accessing cached files
        ...
        model_path = "/app/model_cache/qwen3-vl-32b"
        if not os.path.exists(model_path):
            ...
            print(f"Model cache found, using {model_path}")
        ...
        # Pre-compile DeepGEMM kernels and cache to B10fs persistent storage
        # Uses SGLANG_DG_CACHE_DIR env var set in config.yaml (/cache/org/deep_gemm_qwen3_vl_32b_tp2)
        # First run: compiles (10-20 min), subsequent runs: loads from cache (seconds)
        deep_gemm_cache = os.environ.get("SGLANG_DG_CACHE_DIR", "/cache/org/deep_gemm_qwen3_vl_32b_tp2")
        logger.info(f"Pre-compiling DeepGEMM kernels (cache dir: {deep_gemm_cache})...")
        ...
        try:
            ...
            if compile_result.returncode == 0:
                logger.info(f"DeepGEMM pre-compilation completed in {compile_time:.1f}s")
            else:
                logger.warning(f"DeepGEMM pre-compilation returned non-zero: {compile_result.stderr}")
        except subprocess.TimeoutExpired:
            logger.warning("DeepGEMM pre-compilation timed out after 30 min")
        except Exception as e:
            logger.warning(f"DeepGEMM pre-compilation failed: {e}")
        ...
        # Initialize HTTP client for SGLang with connection pooling
        _init_sglang_client()
    ...
    def _wait_for_sglang(self):
        ...
        # Warmup: trigger DeepGEMM JIT by sending a dummy request
        # This ensures JIT compilation completes before model is marked ready
        logger.info("Sending warmup request to trigger DeepGEMM JIT compilation...")
        try:
            warmup_payload = {
            ...
            response = httpx.post(
                ...
                json=warmup_payload,
                timeout=1200.0  # 20 min timeout for JIT compilation
            )
            warmup_time = time.time() - warmup_start
            logger.info(f"Warmup complete in {warmup_time:.1f}s, status: {response.status_code}")
        except Exception as e:
            logger.warning(f"Warmup request failed (model may still work): {e}")
    ...
    async def predict(self, model_input: Dict[str, Any]) -> Union[Dict[str, Any], AsyncIterator[bytes]]:
        ...
        # === MODEL INFERENCE ===
        client = _init_sglang_client()
        sglang_url = f"http://localhost:{self.sglang_port}"
        
        try:
            if stream:
                # Handle streaming response with retry for initial connection
                async def generate():
                    ...
                    for attempt in range(max_retries):
                        try:
                            if attempt > 0:
                                ...
                                logger.warning(
                                    f"Stream connection failed, retry {attempt + 1}/{max_retries} "
                            ...
                            if attempt > 0:
                                logger.info(f"Stream connection succeeded on attempt {attempt + 1}")
                            ...
                            if attempt >= max_retries - 1:
                                logger.error(f"All {max_retries} stream connection attempts failed: {e}")
                                raise RuntimeError(
                                    f"Failed to connect to SGLang server after {max_retries} attempts"
                                ) from e
