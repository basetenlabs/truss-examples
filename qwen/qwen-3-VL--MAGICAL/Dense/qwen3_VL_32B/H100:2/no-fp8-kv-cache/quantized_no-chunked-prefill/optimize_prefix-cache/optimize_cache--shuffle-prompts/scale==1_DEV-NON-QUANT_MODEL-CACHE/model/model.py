...
class Model:
    ...
    def load(self):
        ...
        # Block until model cache download is complete (required when using model_cache in config)
        if self._lazy_data_resolver:
            logger.info("Waiting for model cache download to complete...")
            self._lazy_data_resolver.block_until_download_complete()
            logger.info("Model cache download complete.")
        ...
        # Determine model path - use model_cache if available
        model_path = "/app/model_cache"
        if not os.path.exists(model_path) or not os.listdir(model_path):
            # Fallback to direct path or HuggingFace download
            model_path = "/app/Qwen3-VL-32B-Instruct"
        ...
        cmd = [
            ...
            "--max-running-requests", "4",        # ← Reduced to match Baseten cap
            "--mm-max-concurrent-calls", "4",
    ...
    def is_healthy(self) -> bool:
        """
        Custom health check that verifies SGLang server is responsive
        and GPU memory utilization is below threshold.
        """
        ...
        try:
            ...
            if result.returncode == 0:
                for line in result.stdout.strip().split("\n"):
                    ...
                    if utilization >= 90:
                        logger.warning(f"GPU memory utilization at {utilization:.1f}% (>= 90%), marking unhealthy")
                        self._is_healthy = False
                        return False
        except Exception as e:
            logger.warning(f"Failed to check GPU memory: {e}")
            # Don't fail health check if nvidia-smi fails - continue to SGLang check
        
        # Check SGLang server health
        try:
            sglang_url = f"http://localhost:{self.sglang_port}"
            response = httpx.get(f"{sglang_url}/health_generate", timeout=5.0)
            # Only fail on 5xx errors (server errors), not 4xx (client errors)
            if response.status_code >= 500:
                logger.warning(f"SGLang health check returned 5xx status: {response.status_code}")
                self._is_healthy = False
                return False
            return True
        except httpx.RequestError as e:
            logger.warning(f"SGLang health check failed with exception: {e}")
            self._is_healthy = False
            return False
        except Exception as e:
    ...
    async def predict(self, model_input: Dict[str, Any]) -> Union[Dict[str, Any], AsyncIterator[bytes]]:
        ...
        request_body = {
            "model": model_input.get("model", "Qwen/Qwen3-VL-32B-Instruct"),
            "messages": messages,
        ...
        try:
            if stream:
                ...
                async def generate():
                    ...
                    logger.info(f"SGLang stream response status: {response.status_code}")
                    try:
                ...
                logger.info(f"SGLang response status: {response.status_code}, inference took {inference_time*1000:.2f}ms")
            ...
            # Connection timeout indicates server may be unhealthy
            self._is_healthy = False
            raise RuntimeError(
            ...
            # HTTP errors (5xx) indicate server issues
            if hasattr(e, 'response') and e.response is not None and e.response.status_code >= 500:
                self._is_healthy = False
            raise RuntimeError(f"HTTP error communicating with SGLang server: {e}") from e
        except Exception as e:
            logger.error(f"Error during generation: {e}")
            # Check for CUDA errors or other critical failures
            error_str = str(e).lower()
            if "cuda" in error_str or "out of memory" in error_str or "device" in error_str:
                self._is_healthy = False
            raise
