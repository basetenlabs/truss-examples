        ...
        for tok_str in split_custom_tokens(token_sim):
            ...
            if count % 7 == 0 and count > 27:
                ...
                while next_yield_idx < len(pending) and pending[next_yield_idx].done():
                    audio = await pending[next_yield_idx]
                    if audio:
                        yield audio
                    next_yield_idx += 1
...
class Model:
    def __init__(self, trt_llm: dict, **kwargs) -> None:
    ...
    def load(self) -> None:
        ...
        self.use_fast_fmt = (
            self._format_prompt_fast("hello world", "tara")
    ...
    def _format_prompt_slow(self, prompt: str, voice="tara") -> str:
    ...
    def _format_prompt_fast(self, prompt: str, voice="tara") -> str:
    ...
    def format_prompt(self, prompt: str, voice="tara") -> str:
    ...
    async def websocket(self, websocket: WebSocket):
        """
        WebSocket endpoint to receive text inputs and stream audio bytes back.
        Each message from client should be a JSON-encoded model_input dict.
        """
        try:
            while True:
                ...
                # Prepare input just like in predict
                ...
                # Invoke the underlying engine
                token_gen = await self._engine.predict(model_input, None)
                if isinstance(token_gen, StreamingResponse):
                    token_gen = token_gen.body_iterator

                attach_header = model_input.get("attach_wav_header", True)
                if attach_header:
                    header = self.create_wav_header(
                        sample_rate=24_000, bits_per_sample=16, channels=1
                    )
            ...
            print(f"WebSocket disconnected for request {req_id}")
