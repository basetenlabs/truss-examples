...
def test_truss_api_call(prompt="a beautiful photograph of Mt. Fuji during cherry blossom, photorealistic, high quality", 
                       height=1024, width=1024, steps=30, guidance_scale=3.5, seed=42):
    ...
    api_key = os.getenv("BASETEN_API_KEY", "YOUR_API_KEY")  # Get from environment variable
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("status") == "success":
                print(f"✅ Truss API prediction successful!")
                print(f"   Time: {end_time - start_time:.2f} seconds")
                print(f"   Prompt: {result.get('prompt', prompt)}")
                print(f"   Dimensions: {result.get('width', width)}x{result.get('height', height)}")
                print(f"   Steps: {result.get('num_inference_steps', steps)}")
                print(f"   Guidance Scale: {result.get('guidance_scale', guidance_scale)}")
                print(f"   Seed: {result.get('seed', seed)}")
                
                return {"result": result, "time": end_time - start_time, "success": True}
            else:
                print(f"❌ Truss API prediction failed: {result.get('error', 'Unknown error')}")
                return {"result": None, "time": end_time - start_time, "success": False, "error": result.get('error', 'Unknown error')}
        else:
            print(f"❌ API request failed with status code: {response.status_code}")
            print(f"   Response: {response.text}")
            return {"result": None, "time": end_time - start_time, "success": False, "error": f"HTTP {response.status_code}"}
...
def test_concurrent_requests(prompts: List[str], max_workers: int = 5, 
                           height=1024, width=1024, steps=30, guidance_scale=3.5, seed=42,
                           sizes: List[tuple] = None):
    """Test Truss API endpoint with concurrent requests."""
    print(f"🚀 Starting concurrent load test with {len(prompts)} requests, max {max_workers} workers")
    if sizes:
    ...
    def make_request(prompt, request_id):
        ...
        if sizes and request_id < len(sizes):
            ...
            print(f"📤 Sending request {request_id + 1}/{len(prompts)} ({h}x{w}): '{prompt[:50]}{'...' if len(prompt) > 50 else ''}'")
            return test_truss_api_call(prompt, h, w, steps, guidance_scale, seed)
        else:
            print(f"📤 Sending request {request_id + 1}/{len(prompts)}: '{prompt[:50]}{'...' if len(prompt) > 50 else ''}'")
            return test_truss_api_call(prompt, height, width, steps, guidance_scale, seed)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_prompt):
            ...
            try:
                ...
                results.append({"result": None, "time": 0, "success": False, "error": str(e)})
...
def get_variable_sizes(num_sizes: int, size_range: str = "512-1024") -> List[tuple]:
    ...
    try:
        ...
        common_sizes = [
            ...
            (1280, 1280), (1280, 1536), (1536, 1280),
            (1536, 1536), (1536, 1792), (1792, 1536),
            (1792, 1792), (1792, 2048), (2048, 1792),
            (2048, 2048)
        ]
        
        # Filter sizes within the specified range
        valid_sizes = [
            (h, w) for h, w in common_sizes 
...
def save_all_images(results: List[Dict[str, Any]], prompts: List[str], output_dir="./output", sizes: List[tuple] = None):
    ...
    for i, result in enumerate(successful_results):
        try:
            ...
            if sizes and i < len(sizes):
                h, w = sizes[i]
                size_info = f"_{h}x{w}"
            elif result.get("result") and "width" in result["result"] and "height" in result["result"]:
    ...
    print(f"✅ Successfully saved {saved_count}/{len(successful_results)} images to '{output_dir}'")
...
def main():
    parser = argparse.ArgumentParser(description="Test the Flux model using Truss API endpoint")
    ...
    parser.add_argument("--api-key", type=str, help="Baseten API key (or set BASETEN_API_KEY environment variable)")
    parser.add_argument("--output-dir", type=str, default="./output", help="Output directory for saved images (default: ./output)")
    
    # Load testing arguments
    parser.add_argument("--concurrent", action="store_true", help="Run concurrent load test")
    parser.add_argument("--num-requests", type=int, default=5, help="Number of concurrent requests (default: 5)")
    parser.add_argument("--max-workers", type=int, default=5, help="Maximum number of concurrent workers (default: 5)")
    parser.add_argument("--prompt-file", type=str, help="File containing prompts (one per line) for load testing")
    parser.add_argument("--use-varied-prompts", action="store_true", help="Use different preset prompts for load testing")
    parser.add_argument("--save-all-images", action="store_true", help="Save all successful images from concurrent load test (automatically enables image saving)")
    parser.add_argument("--variable-sizes", action="store_true", help="Use variable image sizes during load testing")
    parser.add_argument("--size-range", type=str, default="512-1024", help="Range of sizes for variable testing (format: min-max, e.g., 512-1024)")
    ...
    if args.concurrent:
        ...
        if len(prompts) <= 5:
            print(f"📝 Prompts to be tested:")
            for i, prompt in enumerate(prompts):
                size_info = f" ({sizes[i][0]}x{sizes[i][1]})" if sizes and i < len(sizes) else ""
                print(f"   {i+1}. {prompt}{size_info}")
        else:
            print(f"📝 First 3 prompts to be tested:")
