...
class StableCascadePipeline(StableDiffusionPipeline):
    ...
    def initializeModels(self, framework_model_dir, int8, fp8):
        ...
        if "clip" in self.stages:
            self.models["clip"] = CLIPWithProjModel(
                **models_args,
                fp16=self.fp16,
                bf16=self.bf16,
        ...
        if "unet" in self.stages:
            self.models["unet"] = UNetCascadeModel(
                **models_args,
                fp16=self.fp16,
                bf16=self.bf16,
                lite=self.lite,
    ...
    def encode_prompt(
        ...
        def tokenize(prompt, output_hidden_states):
            ...
            attention_mask = text_inputs.attention_mask.type(torch.int32).to(
                self.device
            )
        ...
        if output_hidden_states:
            text_embeddings = (
                torch.cat([text_hidden_states, uncond_hidden_states])
                if self.do_classifier_free_guidance
    ...
    def denoise_latent(
        ...
        with torch.autocast("cuda", enabled=do_autocast):
            ...
            for step_index, timestep in enumerate(timesteps):
                ...
                timestep_ratio_input = (
                    torch.cat([timestep_ratio] * 2)
                    if self.do_classifier_free_guidance
                    else timestep_ratio
                )
                ...
                params = {
                    ...
                    "timestep_ratio": timestep_ratio_input,
                    "clip_text_pooled": pooled_embeddings,
                }
        ...
        if "vqgan" in self.stages:
            print(
                "| {:^15} | {:>9.2f} ms |".format(
                    "VQGAN",
                    cudart.cudaEventElapsedTime(
                        self.events["vqgan"][0], self.events["vqgan"][1]
                    )[1],
                )
            )
        print("|-----------------|--------------|")
    ...
    def infer(
        ...
        if self.pipeline_type.is_cascade_decoder():
            assert image_embeddings is not None, (
                "Image Embeddings are required to run the decoder. Provided None"
            )
        ...
        with torch.inference_mode(), trt.Runtime(TRT_LOGGER):
            ...
            latents = self.initialize_latents(
                ...
                else 4,  # TODO: can we query "in_channels" from config
                latent_height=latent_height
                if self.pipeline_type.is_cascade_prior()
                else int(latent_height * self.latent_dim_scale),
                latent_width=latent_width
                if self.pipeline_type.is_cascade_prior()
                else int(latent_width * self.latent_dim_scale),
                latents_dtype=latents_dtpye,
            )

            # CLIP text encoder(s)
            text_embeddings, pooled_embeddings = self.encode_prompt(
                prompt,
                negative_prompt,
                encoder="clip",
                pooled_outputs=True,
            ...
            if self.pipeline_type.is_cascade_prior():
                ...
                image_embeds_pooled = torch.zeros(
                    batch_size, 1, 768, device=self.device, dtype=latents_dtpye
                )
                image_embeds = (
                    torch.cat(
                        [image_embeds_pooled, torch.zeros_like(image_embeds_pooled)]
                    )
                    if self.do_classifier_free_guidance
