...
class StableDiffusionPipeline:
    ...
    def getOnnxPath(self, model_name, onnx_dir, opt=True, suffix=""):
        onnx_model_dir = os.path.join(
            onnx_dir,
            self.cachedModelName(model_name) + suffix + (".opt" if opt else ""),
        )
    ...
    def getEnginePath(self, model_name, engine_dir, enable_refit=False, suffix=""):
        return os.path.join(
            engine_dir,
            self.cachedModelName(model_name)
            + suffix
    ...
    def getWeightsMapPath(self, model_name, onnx_dir):
        onnx_model_dir = os.path.join(
            onnx_dir, self.cachedModelName(model_name) + ".opt"
        )
        os.makedirs(onnx_model_dir, exist_ok=True)
    ...
    def getStateDictPath(self, model_name, onnx_dir, suffix=""):
        onnx_model_dir = os.path.join(
            onnx_dir, self.cachedModelName(model_name) + suffix
        )
        os.makedirs(onnx_model_dir, exist_ok=True)
    ...
    def initializeModels(self, framework_model_dir, int8, fp8):
        ...
        if "clip2" in self.stages:
            ...
            self.models["clip2"] = CLIPWithProjModel(
                **models_args,
                fp16=True,
                output_hidden_states=self.config.get("clip_hidden_states", False),
                subfolder=subfolder,
    ...
    def loadEngines(
        ...
        **_kwargs,
        ...
        lora_suffix = (
            ...
            + "-".join(
                [
                    str(md5(path.encode("utf-8")).hexdigest())
                    + "-"
                    + ("%.2f" % self.lora_weights[path])
                    + "-"
                    + ("%.2f" % self.lora_loader.scale)
                    for path in sorted(self.lora_loader.paths)
                ]
            )
            if self.lora_loader
            else ""
        ...
        do_engine_refit = dict(
            zip(
                model_names,
                [
                    not self.pipeline_type.is_sd_xl_refiner()
                    and enable_refit
        ...
        torch_fallback = dict(
            zip(model_names, [self.torch_inference for model_name in model_names])
        )
        model_suffix = dict(
            zip(
                model_names,
                [
                    lora_suffix if do_lora_merge[model_name] else ""
                    for model_name in model_names
                ],
            )
        )
        ...
        if int8:
            assert self.pipeline_type.is_sd_xl_base() or self.version in [
                ...
                "int8 quantization only supported for SDXL, SD1.4, SD1.5 and SD2.1 pipeline"
            )
            model_name = "unetxl" if self.pipeline_type.is_sd_xl() else "unet"
            use_int8[model_name] = True
            model_suffix[model_name] += (
                f"-int8.l{quantization_level}.bs2.s{self.denoising_steps}.c{calibration_size}.p{quantization_percentile}.a{quantization_alpha}"
            )
        elif fp8:
            assert self.pipeline_type.is_sd_xl() or self.version in [
                "1.4",
                "1.5",
                "2.1",
                "2.1-base",
            ], (
                "fp8 quantization only supported for SDXL, SD1.4, SD1.5 and SD2.1 pipeline"
            )
            model_name = "unetxl" if self.pipeline_type.is_sd_xl() else "unet"
            use_fp8[model_name] = True
            model_suffix[model_name] += (
                f"-fp8.l{quantization_level}.bs2.s{self.denoising_steps}.c{calibration_size}.p{quantization_percentile}.a{quantization_alpha}"
            )
        onnx_path = dict(
            zip(
                model_names,
                [
                    self.getOnnxPath(
                        model_name, onnx_dir, opt=False, suffix=model_suffix[model_name]
                    )
                    for model_name in model_names
                ],
            )
        ...
        onnx_opt_path = dict(
            zip(
                ...
                [
                    self.getOnnxPath(
                        model_name, onnx_dir, suffix=model_suffix[model_name]
                    )
                    for model_name in model_names
                ],
            )
        ...
        weights_map_path = dict(
            zip(
                model_names,
                [
                    (
                        self.getWeightsMapPath(model_name, onnx_dir)
                        if do_engine_refit[model_name]
                        else None
                    )
                    for model_name in model_names
                ],
        ...
        for model_name, obj in self.models.items():
            ...
            do_export_weights_map = weights_map_path[model_name] and not os.path.exists(
                weights_map_path[model_name]
            )
            if do_export_onnx or do_export_weights_map:
                ...
                if not use_int8[model_name] and not use_fp8[model_name]:
                    obj.export_onnx(
                        ...
                        opt_image_height,
                        opt_image_width,
                        enable_lora_merge=do_lora_merge[model_name],
                    ...
                    state_dict_path = self.getStateDictPath(
                        model_name, onnx_dir, suffix=model_suffix[model_name]
                    )
                    if not os.path.exists(state_dict_path):
                        print(
                            f"[I] Calibrated weights not found, generating {state_dict_path}"
                        )
                        ...
                        calibration_prompts = load_calib_prompts(
                            calib_batch_size, calibration_file
                        )
                        ...
                        print(
                            f"[I] Performing calibration for {calibration_size} steps."
                        )
                    ...
                    print(
                        f"[I] Generating quantized ONNX model: {onnx_opt_path[model_name]}"
                    )
            ...
            if do_export_weights_map:
                ...
                obj.export_weights_map(
                    onnx_opt_path[model_name], weights_map_path[model_name]
                )
        ...
        for model_name, obj in self.models.items():
            ...
            if do_engine_refit[model_name] and self.lora_loader:
                ...
                with open(weights_map_path[model_name], "r") as fp_wts:
                    ...
                    refit_weights_path = self.getRefitNodesPath(
                        model_name, engine_dir, suffix=lora_suffix
                    )
                    if not os.path.exists(refit_weights_path):
                        ...
                        refit_weights, updated_weight_names = (
                            engine_module.get_refit_weights(
                                model.state_dict(),
                                onnx_opt_path[model_name],
                        ...
                        torch.save(
                            (refit_weights, updated_weight_names), refit_weights_path
                        )
    ...
    def preprocess_controlnet_images(self, batch_size, images=None):
        ...
        images = [
            torch.cat([torch.from_numpy(i).to(self.device).float()] * 2) for i in images
        ]
    ...
    def encode_prompt(
        ...
        def tokenize(prompt, output_hidden_states):
            ...
            if self.torch_inference:
                outputs = self.torch_models[encoder](
                    text_input_ids, output_hidden_states=output_hidden_states
                )
        ...
        if self.do_classifier_free_guidance:
            ...
            uncond_embeddings, uncond_hidden_states = tokenize(
                negative_prompt, output_hidden_states
            )
        ...
        if output_hidden_states:
            text_embeddings = (
                torch.cat([uncond_hidden_states, text_hidden_states]).to(
                    dtype=torch.float16
                )
                if self.do_classifier_free_guidance
    ...
    def denoise_latent(
        ...
        controlnet_imgs = self.preprocess_controlnet_images(
            latents.shape[0], controlnet_imgs
        )
    ...
    def encode_image(self, input_image):
        ...
        cast_to = (
            ...
            if self.models["vae_encoder"].bf16
            else torch.float32
        )
        ...
        if self.torch_inference:
            ...
            image_latents = self.runEngine("vae_encoder", {"images": input_image})[
                "latent"
            ]
    ...
    def print_summary(self, denoising_steps, walltime_ms, batch_size):
        ...
        print(
            "| {:^15} | {:>9.2f} ms |".format(
                "UNet"
                + ("+CNet" if self.pipeline_type.is_controlnet() else "")
                + " x "
                + str(denoising_steps),
                cudart.cudaEventElapsedTime(
    ...
    def infer(
        ...
        with torch.inference_mode(), trt.Runtime(TRT_LOGGER):
            ...
            denoise_kwargs = {}
            if not (
                self.pipeline_type.is_img2img() or self.pipeline_type.is_sd_xl_refiner()
            ):
                # Initialize latents
            ...
            if (
                self.pipeline_type.is_img2img()
                or self.pipeline_type.is_inpaint()
                or self.pipeline_type.is_sd_xl_refiner()
            ):
            ...
            if self.pipeline_type.is_img2img() or self.pipeline_type.is_sd_xl_refiner():
                ...
                image_latents = (
                    ...
                    else self.encode_image(input_image)
                )
                ...
                latents = self.scheduler.add_noise(
                    image_latents, noise, latent_timestep
                )
            elif self.pipeline_type.is_inpaint():
                mask, mask_image = self.preprocess_images(
                    batch_size,
                    image_module.prepare_mask_and_masked_image(input_image, mask_image),
                )
            ...
            if self.pipeline_type.is_sd_xl():
                ...
                if self.pipeline_type.is_sd_xl_base():
                    text_embeddings = self.encode_prompt(
                        prompt, negative_prompt, output_hidden_states=True
                    )
                    text_embeddings = torch.cat(
                        [text_embeddings, text_embeddings2], dim=-1
                    )
                else:
                ...
                def _get_add_time_ids(
                    ...
                    if (
                        self.pipeline_type.is_sd_xl_refiner()
                    ):  # self.requires_aesthetics_score:
                        add_time_ids = list(
                    ...
                    add_time_ids = torch.tensor(
                        [add_time_ids], dtype=dtype, device=self.device
                    )
                    if self.do_classifier_free_guidance:
                        add_neg_time_ids = torch.tensor(
                            [add_neg_time_ids], dtype=dtype, device=self.device
                        )
                ...
                denoise_kwargs.update(
                    {"text_embeds": pooled_embeddings2, "time_ids": add_time_ids}
                )
            else:
            ...
            latents = self.denoise_latent(
                latents, text_embeddings, denoiser=denoiser, **denoise_kwargs
            )
