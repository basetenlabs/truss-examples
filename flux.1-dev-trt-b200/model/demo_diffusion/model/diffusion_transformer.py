...
class SD3_MMDiTModel(base_model.BaseModel):
    ...
    def get_input_profile(
        ...
        return {
            ...
            "y": [
                (self.xB * min_batch, 2048),
                (self.xB * batch_size, 2048),
                (self.xB * max_batch, 2048),
            ],
        }
    ...
    def get_sample_input(self, batch_size, image_height, image_width, static_shape):
        ...
        return (
            ...
            {
                "c_crossattn": torch.randn(
                    batch_size, 154, 4096, dtype=dtype, device=self.device
                ),
...
class FluxTransformerModel(base_model.BaseModel):
    def __init__(
        ...
        if not os.path.exists(self.transformer_model_dir):
            self.config = FluxTransformer2DModel.load_config(
                self.path, subfolder=self.subfolder, token=self.hf_token
            )
        else:
            print(
                f"[I] Load FluxTransformer2DModel config from: {self.transformer_model_dir}"
            )
        ...
        self.out_channels = (
            self.config.get("out_channels") or self.config["in_channels"]
        )
    ...
    def get_model(self, torch_inference=""):
        ...
        if not load.is_model_cached(
            self.transformer_model_dir, model_opts, self.hf_safetensor
        ):
            ...
            print(
                f"[I] Load FluxTransformer2DModel model from: {self.transformer_model_dir}"
            )
            model = FluxTransformer2DModel.from_pretrained(
                self.transformer_model_dir, **model_opts
            ).to(self.device)
        if torch_inference:
    ...
    def get_input_profile(
        ...
        input_profile = {
            "hidden_states": [
                (
                    min_batch,
                    (min_latent_height // 2) * (min_latent_width // 2),
                ...
                (
                    max_batch,
                    (max_latent_height // 2) * (max_latent_width // 2),
                    self.config["in_channels"],
                ),
            ],
            "encoder_hidden_states": [
    ...
    def get_sample_input(self, batch_size, image_height, image_width, static_shape):
        ...
        assert not (self.fp16 and self.bf16), (
            "fp16 and bf16 cannot be enabled simultaneously"
        )
        tensor_dtype = (
            torch.bfloat16
            if self.bf16
        ...
        if self.config["guidance_embeds"]:
            sample_input[-1]["guidance"] = torch.tensor(
                [1.0] * batch_size, dtype=dtype, device=self.device
            )
        return sample_input
...
class UpcastLayer(torch.nn.Module):
    ...
    def forward(self, *inputs, **kwargs):
        casted_inputs = tuple(
            in_val.to(self.upcast_to) if isinstance(in_val, torch.Tensor) else in_val
        ...
        for name, val in kwargs.items():
            kwarg_casted[name] = (
                val.to(dtype=self.upcast_to) if isinstance(val, torch.Tensor) else val
            )

        output = self.base_layer(*casted_inputs, **kwarg_casted)
        if isinstance(output, tuple):
            output = tuple(
                out.to(self.output_dtype) if isinstance(out, torch.Tensor) else out
                for out in output
            )
        else:
...
class SD3TransformerModel(base_model.BaseModel):
    def __init__(
        ...
        if not os.path.exists(self.transformer_model_dir):
            self.config = SD3Transformer2DModel.load_config(
                self.path, subfolder=self.subfolder, token=self.hf_token
            )
        else:
            print(
                f"[I] Load SD3Transformer2DModel config from: {self.transformer_model_dir}"
            )
    ...
    def get_model(self, torch_inference=""):
        ...
        if not load.is_model_cached(
            ...
            print(
                f"[I] Load SD3Transformer2DModel model from: {self.transformer_model_dir}"
            )
            model = SD3Transformer2DModel.from_pretrained(
                self.transformer_model_dir, **model_opts
            ).to(self.device)
        ...
        if self.version == "3.5-large":
            model.transformer_blocks[35] = UpcastLayer(
                model.transformer_blocks[35], torch.float32
            )
