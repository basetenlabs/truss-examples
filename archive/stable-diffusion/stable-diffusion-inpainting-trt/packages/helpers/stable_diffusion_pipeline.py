...
class StableDiffusionPipeline:
    ...
    def getOnnxPath(self, model_name, onnx_dir, opt=True):
        return os.path.join(
            onnx_dir,
            self.cachedModelName(model_name) + (".opt" if opt else "") + ".onnx",
        )
    ...
    def loadEngines(
        ...
        if "vae_encoder" in self.stages:
            self.models["vae_encoder"] = make_VAEEncoder(
                inpaint=self.inpaint, **models_args
            )
    ...
    def preprocess_images(self, batch_size, images=()):
        if self.nvtx_profile:
            nvtx_image_preprocess = nvtx.start_range(
                message="image_preprocess", color="pink"
            )
        init_images = []
        for image in images:
    ...
    def encode_prompt(self, prompt, negative_prompt):
        ...
        uncond_embeddings = self.runEngine("clip", {"input_ids": uncond_input_ids_inp})[
            "text_embeddings"
        ]
    ...
    def denoise_latent(
        ...
        for step_index, timestep in enumerate(timesteps):
            ...
            latents = self.scheduler.step(
                noise_pred, latents, step_offset + step_index, timestep
            )
            ...
            print(
                "| {:^10} | {:>9.2f} ms |".format(
                    ...
                    cudart.cudaEventElapsedTime(
                        self.events["vae_encoder-start"],
                        self.events["vae_encoder-stop"],
                    )[1],
                )
            )
        print(
            "| {:^10} | {:>9.2f} ms |".format(
                "CLIP",
                cudart.cudaEventElapsedTime(
                    self.events["clip-start"], self.events["clip-stop"]
                )[1],
            )
        )
        print(
            "| {:^10} | {:>9.2f} ms |".format(
                ...
                cudart.cudaEventElapsedTime(
                    self.events["denoise-start"], self.events["denoise-stop"]
                )[1],
            )
        )
        print(
            "| {:^10} | {:>9.2f} ms |".format(
                "VAE-Dec",
                cudart.cudaEventElapsedTime(
                    self.events["vae-start"], self.events["vae-stop"]
                )[1],
            )
        )
        print("|------------|--------------|")
    ...
    def save_image(self, images, pipeline, prompt):
        ...
        output_paths = save_image(images, self.output_dir, image_name_prefix)
        return output_paths
