...
class LMSDiscreteScheduler:
    ...
    def set_timesteps(self, steps):
        ...
        timesteps = np.linspace(0, self.num_train_timesteps - 1, steps, dtype=float)[
            ::-1
        ].copy()
        sigmas = np.array(((1 - self.alphas_cumprod) / self.alphas_cumprod) ** 0.5)
    ...
    def scale_model_input(
        self, sample: torch.FloatTensor, idx, *args, **kwargs
    ) -> torch.FloatTensor:
    ...
    def configure(self):
        ...
        for step_index in range(self.num_inference_steps):
            ...
            self.lms_coeffs.append(
                [
                    get_lms_coefficient(order, step_index, curr_order)
    ...
    def step(self, output, latents, idx, timestep):
        ...
        if self.prediction_type == "epsilon":
            ...
            # * c_out + input * c_skip
            pred_original_sample = output * (-sigma / (sigma**2 + 1) ** 0.5) + (
                latents / (sigma**2 + 1)
            )
        else:
            raise ValueError(
        ...
        prev_sample = latents + sum(
            ...
            for coeff, derivative in zip(
                self.lms_coeffs[idx], reversed(self.derivatives)
            )
        )
...
class DDIMScheduler:
    ...
    def step(
        ...
        alpha_prod_t_prev = (
            ...
            if prev_idx < self.num_inference_steps
            else self.final_alpha_cumprod
        )
    ...
    def add_noise(self, init_latents, noise, idx, latent_timestep):
        ...
        noisy_latents = (
            sqrt_alpha_prod * init_latents + sqrt_one_minus_alpha_prod * noise
        )
...
class DPMScheduler:
    ...
    def configure(self):
        ...
        for step_index in range(self.num_inference_steps):
            ...
            prev_timestep = (
                ...
                else self.timesteps[step_idx + 1]
            )

            self.dpm_solver_first_order_coefs_precompute(timestep, prev_timestep)

            timestep_list = [self.timesteps[step_index - 1], timestep]
            self.multistep_dpm_solver_second_order_coefs_precompute(
                timestep_list, prev_timestep
            )
            ...
            self.multistep_dpm_solver_third_order_coefs_precompute(
                timestep_list, prev_timestep
            )
    ...
    def multistep_dpm_solver_second_order_coefs_precompute(
        self, timestep_list, prev_timestep
    ):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if self.solver_type == "midpoint":
                ...
                self.second_order_third_coef.append(
                    0.5 * (alpha_t * (torch.exp(-h) - 1.0))
                )
    ...
    def multistep_dpm_solver_third_order_coefs_precompute(
        self, timestep_list, prev_timestep
    ):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            self.third_order_third_coef.append(
                alpha_t * ((torch.exp(-h) - 1.0) / h + 1.0)
            )
            self.third_order_fourth_coef.append(
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if self.thresholding:
                ...
                dynamic_max_val = torch.quantile(
                    torch.abs(x0_pred).reshape((x0_pred.shape[0], -1)),
                ...
                dynamic_max_val = torch.maximum(
                    ...
                    * torch.ones_like(dynamic_max_val).to(dynamic_max_val.device),
                )[(...,) + (None,) * (x0_pred.ndim - 1)]
                x0_pred = (
                    torch.clamp(x0_pred, -dynamic_max_val, dynamic_max_val)
                    / dynamic_max_val
                )
            return x0_pred
    ...
    def multistep_dpm_solver_second_order_update(
        self, idx, model_output_list, timestep_list, prev_timestep, sample
    ):
        t, s0, s1 = prev_timestep, timestep_list[-1], timestep_list[-2]
...
class PNDMScheduler:
    ...
    def set_timesteps(self, steps):
        ...
        plms_timesteps = np.concatenate(
            [timesteps[:-1], timesteps[-2:-1], timesteps[-1:]]
        )[::-1].copy()
    ...
    def configure(self):
        self.alphas_cumprod_prev = torch.roll(
            self.alphas_cumprod, shifts=self.step_ratio
        )
        ...
        self.model_output_denom_coeff = self.alphas_cumprod * (
    ...
    def step(self, output, sample, idx, timestep):
        ...
        if self.prediction_type == "v_prediction":
            output = (self.alphas_cumprod[idx] ** 0.5) * output + (
                self.beta_cumprod[idx] ** 0.5
            ) * sample
        ...
        prev_sample = (
            self.sample_coeff[idx] * sample
            - (self.alphas_cumprod_prev[idx] - self.alphas_cumprod[idx])
            * output
