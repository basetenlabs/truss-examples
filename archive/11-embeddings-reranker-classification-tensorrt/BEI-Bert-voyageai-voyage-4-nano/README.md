# BEI-Bert (Baseten-Embeddings-Inference-BERT) with voyageai/voyage-4-nano

This is a Deployment for BEI-Bert (Baseten-Embeddings-Inference-BERT) with voyageai/voyage-4-nano. BEI is Baseten's solution for production-grade deployments via TensorRT-LLM for (text) embeddings, reranking models and prediction models.
...
# Examples:
This deployment is specifically designed for the Hugging Face model [voyageai/voyage-4-nano](https://huggingface.co/voyageai/voyage-4-nano).
...
voyageai/voyage-4-nano  is a text-embeddings model, producing a 1D embeddings vector, given an input.
...
cd 11-embeddings-reranker-classification-tensorrt/BEI-Bert-voyageai-voyage-4-nano
```

With `11-embeddings-reranker-classification-tensorrt/BEI-Bert-voyageai-voyage-4-nano` as your working directory, you can deploy the model with the following command. Paste your Baseten API key if prompted.

```sh
truss push --publish
# prints:
# ✨ Model BEI-Bert-voyageai-voyage-4-nano-truss-example was successfully pushed ✨
...
model_metadata:
  example_model_input:
    ...
    input: text string
    model: model
model_name: BEI-Bert-voyageai-voyage-4-nano-truss-example
python_version: py39
resources:
  accelerator: L4
  cpu: '1'
